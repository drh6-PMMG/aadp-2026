...
with open(AVALIACOES_FILE, encoding="cp1252", errors="replace") as f:
    ...
    for row in reader:
        ...
        rows.append({
            ...
            "Competência 1": row[14].strip(), "Conceito Comp.1": row[15].strip(), "Nota Comp.1": row[16].strip(),
            "Competência 2": row[17].strip(), "Conceito Comp.2": row[18].strip(), "Nota Comp.2": row[19].strip(),
            "Competência 3": row[20].strip(), "Conceito Comp.3": row[21].strip(), "Nota Comp.3": row[22].strip(),
            "Competência 4": row[23].strip(), "Conceito Comp.4": row[24].strip(), "Nota Comp.4": row[25].strip(),
...
def _write_analise_sheet(ws, df, titulo, s):
    ...
    # Rótulos (somente valores e linha de ligação)
...
if args.mode in ("all", "geral"):
    ...
    xlsx_bytes = _build_workbook(df_full, "GERAL — AADP 2026", df_full)
...
if args.mode in ("all", "units"):
    ...
    for rpm in target_rpms:
        ...
        xlsx_bytes = _build_workbook(df_rpm, rpm, df_full)
