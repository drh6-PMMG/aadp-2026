...
/* BotÃµes do Sidebar - Efeito Cristal LÃ­quido SecundÃ¡rio (Inativo) */
...
/* BotÃµes do Sidebar - Efeito Cristal LÃ­quido PrimÃ¡rio (Ativo) */
...
STACK_ORDER  = ["Encerrada","Aberta","Parcialmente Encerrada","HomologaÃ§Ã£o"]

# âââââââââââââââââââââââ CONFIGURAÃÃO âââââââââââââââââââââââââââââââââââââââââ
def load_config():
...
def calc_cert(j, l):
    ...
    c = concordam(j, l)
    return "NÃO" if c is True else ("SIM" if c is False else "-")
...
def _parse_csv(av_f: str, si_f: str) -> pd.DataFrame:
    ...
    with open(av_f, encoding="cp1252", errors="replace") as f:
        ...
        for row in reader:
            ...
            sc = "ComissÃ£o Atual" if local.upper().strip() == sigef.get(nrpm.lstrip("0") or "0","").upper().strip() else "Nota ProvisÃ³ria"
            rows.append({
                "nrPM (Avaliado)":          nrpm,
                ...
                "Status AvaliaÃ§Ã£o":  calc_status(j, l, n),
            })
    return pd.DataFrame(rows)

@st.cache_data(show_spinner="â³ Carregando e processando dados...")
...
def safe_df(styled_or_df, height=520):
    if hasattr(styled_or_df, "data"):
        if styled_or_df.data.size <= MAX_STYLE:
            ...
            st.info("â¹ï¸ Tabela grande â exibida sem cores para melhor desempenho.")
...
# âââââââââââââââââââââââ SEGURANÃA E AUTENTICAÃÃO ââââââââââââââââââââââââââââââ
...
if not st.session_state.authenticated:
    ...
    with c2:
        ...
        if "auth_mode" not in st.session_state:
            st.session_state.auth_mode = "ð Acessar Conta"
            
        auth_mode = st.radio("Selecione uma opÃ§Ã£o:", ["ð Acessar Conta", "ð Solicitar Cadastro"], horizontal=True, key="auth_mode_radio")
        st.markdown("---")
        
        if auth_mode == "ð Acessar Conta":
                ...
                if not spm or not spass:
                    ...
                    if res:
                        ...
                        if status == "Ativo":
                            ...
                            st.warning("â ï¸ Sua conta estÃ¡ aguardando liberaÃ§Ã£o do Administrador.")
                        else:
                            st.error("â Acesso revogado/bloqueado. Entre em contato com a DRH.")
                    else:
                        st.error("â NÂº PM ou Senha incorretos.")
                        
        else:
            st.markdown("##### ð SolicitaÃ§Ã£o de acesso - Painel de Controle AADP")
            st.info("â ï¸ Informe apenas os **6 primeiros dÃ­gitos** do seu NÂº PM (sem o dÃ­gito verificador).")
            ...
            with col_cons:
                if st.button("ð Consultar", use_container_width=True, type="secondary"):
                    if not reg_pm or not reg_pm.isdigit():
                        ...
                        with st.spinner("â³ Consultando banco de dados do SIGEF..."):
                            ...
                            if res:
                                ...
                                st.success("â Militar encontrado no SIGEF! Prossiga com a verificaÃ§Ã£o de seguranÃ§a abaixo.")
                            else:
                                st.error("â NÂº PM nÃ£o encontrado no banco SIGEF. Verifique se digitou os 6 primeiros dÃ­gitos corretamente.")
                                st.session_state.sigef_data = None
                                st.session_state.sigef_verified = False
            with col_clear:
                if st.button("ð§¹ Limpar", use_container_width=True):
                    st.session_state.sigef_data = None
                    st.session_state.sigef_verified = False
                    st.rerun()
            ...
            if st.session_state.sigef_data:
                ...
                if not st.session_state.sigef_verified:
                    st.markdown("---")
                    st.markdown("##### ð VerificaÃ§Ã£o de SeguranÃ§a")
                    ...
                    if st.button("Confirmar Dados", use_container_width=True, type="primary"):
                        ...
                        if clean_input_cpf == clean_sigef_cpf and clean_input_birth == clean_sigef_birth:
                            ...
                            st.success("â Identidade confirmada com sucesso!")
                            st.rerun()
                        else:
                            st.error("â CPF ou Data de Nascimento incorretos. Verifique suas informaÃ§Ãµes e tente novamente.")
                
                # Etapa 3: LiberaÃ§Ã£o do FormulÃ¡rio de Senha
                if st.session_state.sigef_verified:
                    st.markdown("---")
                    st.markdown("##### ð¤ Dados Funcionais Confirmados:")
                    ...
                    st.markdown("##### ð ConfiguraÃ§Ã£o de Senha de Acesso:")
                            ...
                            try:
                                ...
                                if current_status:
                                    if current_status in ("Pendente", "Ativo"):
                                        st.error("â Este NÂº PM jÃ¡ possui solicitaÃ§Ã£o de acesso ativa ou pendente no sistema!")
                                    else:
                                        ...
                                        st.success("â Nova solicitaÃ§Ã£o enviada com sucesso! Aguarde a liberaÃ§Ã£o do Administrador.")
                                        st.session_state.sigef_data = None
                                        st.session_state.sigef_verified = False
                                else:
                                    h_pass = hashlib.sha256(spass.encode()).hexdigest()
                                    ...
                                    st.success("â SolicitaÃ§Ã£o enviada com sucesso! Aguarde a liberaÃ§Ã£o do Administrador.")
                                    st.session_state.sigef_data = None
                                    st.session_state.sigef_verified = False
                                    st.session_state.sigef_verified = False
                            except Exception as e:
...
with st.sidebar:
    ...
    st.markdown(f"<small>ð¤ <b>Militar:</b> {st.session_state.user_name} ({st.session_state.user_pm})</small>", unsafe_allow_html=True)
    if st.session_state.get("simulation_active", False) and st.session_state.user_role == "ADMINISTRADOR":
        st.markdown(f"<small>ð <b>Perfil Real:</b> ADMINISTRADOR</small>", unsafe_allow_html=True)
        st.markdown(f"<small>ðµï¸ <b>Simulado:</b> <span style='color:#ff9f43;'>{st.session_state.simulated_role}</span></small>", unsafe_allow_html=True)
    else:
        st.markdown(f"<small>ð <b>Perfil:</b> <span style='color:#bca374;'>{sidebar_active_role}</span></small>", unsafe_allow_html=True)
    ...
    btn_filtros_label = "ð Ocultar Filtros" if st.session_state.show_filtros else "ð Mostrar Filtros"
    ...
    st.markdown("#### ð§­ PÃ¡ginas")
    pages = [
        ("ð AnÃ¡lise GrÃ¡fica", "AnÃ¡lise GrÃ¡fica"),
        ("ð Dados Gerais", "Dados Gerais"),
        ("â³ AvaliaÃ§Ãµes Pendentes", "AvaliaÃ§Ãµes Pendentes"),
        ("ð¥ Avaliadores Pendentes", "Avaliadores Pendentes"),
    ]
    # P1 e SADM nÃ£o possuem acesso Ã s opÃ§Ãµes de exportaÃ§Ã£o/relatÃ³rios
    if sidebar_active_role not in ("P1", "SADM"):
        pages.append(("ð¥ Gerar RelatÃ³rio", "Gerar RelatÃ³rio"))
        pages.append(("ð RelatÃ³rio Word", "RelatÃ³rio Word"))
    ...
    if st.session_state.user_role == "ADMINISTRADOR":
        ...
        if pending_count > 0:
            pages.append((f"âï¸ Painel Administrador (ð´ :red[{pending_count}])", "Painel Administrador"))
        else:
            pages.append(("âï¸ Painel Administrador", "Painel Administrador"))
    ...
    if st.session_state.active_page in ("Gerar RelatÃ³rio", "RelatÃ³rio Word") and sidebar_active_role in ("P1", "SADM"):
        st.session_state.active_page = "AnÃ¡lise GrÃ¡fica"
    ...
    db_path = ""
    fonte = cfg.get("fonte_dados", "ð Pasta local / Servidor")
    reload = False

    # Recarregar Dados para Administrador
    if st.session_state.user_role == "ADMINISTRADOR":
        st.markdown("---")
        if st.button("ð Recarregar Dados", use_container_width=True, type="primary", key="btn_reload"):
            st.cache_data.clear()
    ...
    st.markdown("---")
    if st.button("ð Alterar Senha", use_container_width=True, key="btn_toggle_change_password"):
        st.session_state.show_change_password = not st.session_state.get("show_change_password", False)
        st.rerun()

    if st.button("ðª Sair / Logoff", use_container_width=True, key="btn_logoff"):
...
if data_ok:
    ...
    all_status= ["Aberta","Parcialmente Encerrada","HomologaÃ§Ã£o","Encerrada"]
    all_sit   = ["ComissÃ£o Atual","Nota ProvisÃ³ria"]
    all_cert  = ["SIM","NÃO","-"]
    with container_filtros:
        if st.session_state.show_filtros:
            st.markdown("#### ð Filtros de VisualizaÃ§Ã£o")
            if active_role not in ("P1", "SADM"):
                rpm_filter = st.multiselect("ð¢ Unidade RPM", all_rpm, placeholder="Todas")
            else:
                rpm_filter = []
            ...
            if active_role != "SADM":
                unid_filter = st.multiselect("ðï¸ Subunidade", all_unid, placeholder="Todas")
            else:
                unid_filter = []
                
            st.markdown("")
            sit_com_filter = st.multiselect("ðµ SituaÃ§Ã£o ComissÃ£o", all_sit, placeholder="Todas")
            status_filter  = st.multiselect("ð Status",            all_status, placeholder="Todos")
            cert_filter    = st.multiselect("â Cert. Homologador",  all_cert,   placeholder="Todos")
            st.markdown("---")
            st.markdown(f"<small>ð Carregado: {ts}</small>", unsafe_allow_html=True)
            st.markdown(f"<small>ð {fmt_num(len(df_full))} registros</small>", unsafe_allow_html=True)
...
<div class="main-title">
  ...
  <div style="margin-top: 10px;">
    ...
    <p style="font-size: 1.05rem; margin: 5px 0 0 0; color: #e5dccb; font-weight: 500;">PolÃ­cia Militar de Minas Gerais Â· ResoluÃ§Ã£o 5458/2025</p>
...
if st.session_state.get("show_change_password", False):
    st.markdown('<div class="info-box" style="border-left: 5px solid #bca374;">ð¡ï¸ <b>Alterar Senha do UsuÃ¡rio</b></div>', unsafe_allow_html=True)
    with st.form("form_change_password", clear_on_submit=True):
        ...
        submit_chg = st.form_submit_button("ð¾ Atualizar Senha", use_container_width=True, type="primary")
            
    if submit_chg:
        if not curr_pw or not new_pw or not conf_pw:
            st.error("â Por favor, preencha todos os campos.")
        elif new_pw != conf_pw:
            st.error("â A nova senha e a confirmaÃ§Ã£o nÃ£o coincidem.")
        else:
            ...
            if not row_pw or row_pw != h_curr:
                st.error("â Senha atual incorreta.")
            else:
                ...
                st.success("â Senha alterada com sucesso!")
                st.session_state.show_change_password = False
                st.rerun()
                
    if st.button("â Cancelar / Fechar", use_container_width=True, key="btn_cancel_change_pw"):
        st.session_state.show_change_password = False
        st.rerun()
    st.markdown("---")

if not data_ok:
    st.error(f"â {err_msg}")
    st.markdown(f"""<div class="info-box">
    ð Configure a pasta dos CSVs na barra lateral.<br>
    ð Pasta padrÃ£o criada: <code>{DADOS_DIR}</code><br>
...
n_sim    = (df["CertificaÃ§Ã£o Homologador"]=="SIM").sum()
...
st.markdown(f'<div class="info-box">ð Exibindo {fmt_num(n_total)} avaliaÃ§Ãµes {ft}</div>', unsafe_allow_html=True)

col_block1, col_block2 = st.columns([1, 1.25], gap="large")

with col_block1:
    # Bloco 1: Total em cima, comissÃ£o/provisÃ³ria embaixo
    ...
    with cb1_1:
        st.markdown('<div class="kpi-card kpi-ca">'
                    '<div class="label">COMISSÃO ATUAL</div>'
...
with col_block2:
    # Bloco 2: Abertas, Parc, HomologaÃ§Ã£o em cima, Encerradas embaixo
    ...
    with cb2_3:
        st.markdown('<div class="kpi-card kpi-hom">'
                    '<div class="label">HOMOLOGAÃÃO</div>'
...
# âââââââââââââââââââââââ SELEÃÃO DE ABAS VIA SESSION STATE âââââââââââââââââââ
if "active_page" not in st.session_state:
    st.session_state.active_page = "AnÃ¡lise GrÃ¡fica"
...
if active_page == "AnÃ¡lise GrÃ¡fica":
    st.markdown("### ð AnÃ¡lise GrÃ¡fica das AvaliaÃ§Ãµes")

    # ââ LINHA 1: Pizza de Status (destaque, full width) ââââââââââââââââââââââ
    st.markdown("---")
    ordered_labels = ["Aberta","Parcialmente Encerrada","HomologaÃ§Ã£o","Encerrada"]
    ...
    fig_status.add_trace(go.Pie(
        ...
        hovertemplate="<b>%{label}</b><br>AvaliaÃ§Ãµes: <b>%{value:,}</b><br>%{percent}<extra></extra>",
        sort=False,
    ))
    ...
    with c1:
        ...
        fig_sit = go.Figure(go.Pie(
            ...
            hovertemplate="<b>%{label}</b><br>%{value:,} avaliaÃ§Ãµes (%{percent})<extra></extra>",
        ))
        fig_sit.update_layout(
            template="plotly_dark",
            title=dict(text="<b>SituaÃ§Ã£o da ComissÃ£o</b>", font_size=15, x=0.5, font=dict(color="#bca374")),
    ...
    rpm_cross = df.groupby(["Unidade RPM (Avaliado)","Status AvaliaÃ§Ã£o"]).size().reset_index(name="Qtd")
    fig_rpm = px.bar(
        ...
        template="plotly_dark",
        title="<b>DistribuiÃ§Ã£o por Unidade RPM e Status</b>",
        category_orders={
    ...
    with c3:
        ...
        cert_map = {"SIM":"#FF6B6B","NÃO":"#70AD47","-":"#AAAAAA"}
    ...
    with c4:
        ...
        if timeline_frames:
            ...
            fig_time = px.line(
                df_time, x="Data", y="Qtd", color="FunÃ§Ã£o",
...
if active_page == "Dados Gerais":
    st.markdown(f"### ð Dados Gerais â {fmt_num(len(df))} avaliaÃ§Ãµes")
    cols_d = [
        # Avaliado
    ...
    st.download_button("â¬ï¸ Baixar dados filtrados (CSV)", csv_d.encode("utf-8-sig"),
...
if active_page == "AvaliaÃ§Ãµes Pendentes":
    ...
    STATUS_PEND = {"HomologaÃ§Ã£o","Parcialmente Encerrada","Aberta"}
    df_pend = df[df["Status AvaliaÃ§Ã£o"].isin(STATUS_PEND)].copy()

    c1, c2 = st.columns(2)
    with c1:
        tipo_pend = st.multiselect("Status:", ["Aberta","Parcialmente Encerrada","HomologaÃ§Ã£o"],
                                   default=["Aberta","Parcialmente Encerrada","HomologaÃ§Ã£o"],
                                   key="tp")
    with c2:
        sc_pend = st.multiselect("SituaÃ§Ã£o:", ["ComissÃ£o Atual","Nota ProvisÃ³ria"],
                                  default=["ComissÃ£o Atual","Nota ProvisÃ³ria"], key="sp")

    df_pv = df_pend[df_pend["Status AvaliaÃ§Ã£o"].isin(tipo_pend) &
                    df_pend["SituaÃ§Ã£o ComissÃ£o"].isin(sc_pend)] if tipo_pend and sc_pend else df_pend

    k1,k2,k3,k4 = st.columns(4)
    k1.metric("ð´ Abertas",          (df_pv["Status AvaliaÃ§Ã£o"]=="Aberta").sum())
    k2.metric("ð  Parc. Encerradas", (df_pv["Status AvaliaÃ§Ã£o"]=="Parcialmente Encerrada").sum())
    k3.metric("ð¡ HomologaÃ§Ã£o",       (df_pv["Status AvaliaÃ§Ã£o"]=="HomologaÃ§Ã£o").sum())
    k4.metric("ð Total",             len(df_pv))
    ...
    csv_p = df_pv[cols_pend].sort_values(
        ["Unidade RPM (Avaliado)", "Status AvaliaÃ§Ã£o", "Nome (Avaliado)"]
    ).to_csv(index=False, sep=";", encoding="utf-8-sig")
    st.download_button("â¬ï¸ Baixar pendentes (CSV)", csv_p.encode("utf-8-sig"),
...
if active_page == "Avaliadores Pendentes":
    st.markdown("### ð¥ Avaliadores Pendentes")

    if rpm_filter:
        df_ab = df_full[(df_full["Status AvaliaÃ§Ã£o"] == "Aberta") & (df_full["RPM (Av1)"].isin(rpm_filter))]
        df_pe = df_full[(df_full["Status AvaliaÃ§Ã£o"].isin(["Aberta", "Parcialmente Encerrada"])) & (df_full["RPM (Av2)"].isin(rpm_filter))]
        df_hom = df_full[(df_full["Status AvaliaÃ§Ã£o"] == "HomologaÃ§Ã£o") & (df_full["RPM (Hom)"].isin(rpm_filter))]
    else:
        df_ab = df[df["Status AvaliaÃ§Ã£o"] == "Aberta"]
        df_pe = df[df["Status AvaliaÃ§Ã£o"].isin(["Aberta", "Parcialmente Encerrada"])]
        df_hom = df[df["Status AvaliaÃ§Ã£o"] == "HomologaÃ§Ã£o"]
    ...
    # AV1
    st.markdown('<div class="section-hdr">ð¤ AVALIADOR 1 â AvaliaÃ§Ãµes Em Aberto</div>', unsafe_allow_html=True)
    ...
    if not tb1.empty:
        ...
        if has_select:
            st.write("ð¡ *Dica: Clique em uma linha da tabela abaixo para abrir as avaliaÃ§Ãµes deste avaliador.*")
            event1 = st.dataframe(
                tb1_disp,
            ...
            sel_nome = st.selectbox("ð Selecione um Avaliador 1 para ver as avaliaÃ§Ãµes:", ["-- Selecione --"] + list(tb1["Nome"].unique()), key="sel_tb1")
        ...
        if selected_pm:
            ...
            st.markdown(f"#### ð AvaliaÃ§Ãµes pendentes de AV1: **{selected_nome}** ({selected_pm})")
        ...
        st.download_button("â¬ï¸ AV1 (CSV)", tb1.to_csv(index=False,sep=";",encoding="utf-8-sig").encode("utf-8-sig"),
                            f"av1_{now_br().strftime('%Y%m%d_%H%M')}.csv", mime="text/csv")
    else: st.success("â Nenhum AV1 com pendÃªncias!")

    # AV2
    st.markdown('<div class="section-hdr">ð¤ AVALIADOR 2 â Em Aberto + Parcialmente Encerrada</div>', unsafe_allow_html=True)
    ...
    if not tb2.empty:
        ...
        if has_select:
            ...
            sel_nome2 = st.selectbox("ð Selecione um Avaliador 2 para ver as avaliaÃ§Ãµes:", ["-- Selecione --"] + list(tb2["Nome"].unique()), key="sel_tb2")
        ...
        if selected_pm2:
            ...
            st.markdown(f"#### ð AvaliaÃ§Ãµes pendentes de AV2: **{selected_nome2}** ({selected_pm2})")
        ...
        st.download_button("â¬ï¸ AV2 (CSV)", tb2.to_csv(index=False,sep=";",encoding="utf-8-sig").encode("utf-8-sig"),
                            f"av2_{now_br().strftime('%Y%m%d_%H%M')}.csv", mime="text/csv")
    else: st.success("â Nenhum AV2 com pendÃªncias!")

    # HOMOLOGADOR
    st.markdown(
        '<div class="section-hdr-hom">ðï¸ HOMOLOGADOR â AvaliaÃ§Ãµes com DivergÃªncia Aguardando Nota de HomologaÃ§Ã£o</div>',
        unsafe_allow_html=True)
    ...
    if not tb3.empty:
        ...
        if has_select:
            st.write("ð¡ *Dica: Clique em uma linha da tabela abaixo para abrir as avaliaÃ§Ãµes deste homologador.*")
            event3 = st.dataframe(
                tb3_disp,
                use_container_width=True,
                height=280,
            ...
            sel_nome3 = st.selectbox("ð Selecione um Homologador para ver as avaliaÃ§Ãµes:", ["-- Selecione --"] + list(tb3["Nome"].unique()), key="sel_tb3")
        ...
        if selected_pm3:
            ...
            st.markdown(f"#### ð AvaliaÃ§Ãµes pendentes do Homologador: **{selected_nome3}** ({selected_pm3})")
        ...
        st.success("â Nenhum Homologador com pendÃªncias!")

    # ââ Lista detalhada das avaliaÃ§Ãµes em HomologaÃ§Ã£o ââââââââââââââââââââââââââ
    if not df_hom.empty:
        with st.expander(f"ð Ver {len(df_hom)} avaliaÃ§Ã£o(Ãµes) pendentes de homologaÃ§Ã£o", expanded=False):
...
def _write_resumo_sheet(ws, df, titulo, s):
    ...
    headers = ["STATUS / SITUAÃÃO", "COMISSÃO ATUAL â", "NOTA PROVISÃRIA â ï¸",
    ...
    for ci, (txt, fill, fnt) in enumerate([
        ("COMISSÃO ATUAL â Policial estÃ¡ lotado na unidade avaliadora (CA)", fill_ca, white_bold),
...
def _write_analise_sheet(ws, df, titulo, s):
    ...
    r = 3
    headers = ["STATUS", "COMISSÃO ATUAL", "NOTA PROVISÃRIA", "TOTAL", "%"]
    ...
    # Legenda
    for txt, fll, fnt in [
        ("COMISSÃO ATUAL â Policial lotado na unidade avaliadora", fill_ca, white_bold),
...
def _build_workbook(df_unit: pd.DataFrame, titulo: str, df_global: pd.DataFrame = None) -> bytes:
    ...
    df_pend = df_unit[df_unit["Status AvaliaÃ§Ã£o"].isin(["Aberta", "Parcialmente Encerrada", "HomologaÃ§Ã£o"])]
