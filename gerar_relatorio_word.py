...
def generate_word_report(df_source: pd.DataFrame, report_mode: str, selected_rpms: list = None, user_role: str = "ADMINISTRADOR") -> bytes:
    ...
    if show_state_summary:
        ...
        sub_100 = sub_stats[sub_stats["pendentes"] == 0].sort_values(
            by=["Unidade RPM (Avaliado)", "Unidade Principal (Avaliado)"],
            key=lambda col: col.map(rpm_sort_key) if col.name == "Unidade RPM (Avaliado)" else col
        )
        ...
        # Ordenar da MAIOR para a MENOR pendência (descendente)
        sub_pend = sub_pend.sort_values("pendentes", ascending=False)
